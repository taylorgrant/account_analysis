---
title: "Twitter Analysis of: `r handle` | `r handle2` | `r handle3`"
output: 
...
tl3 <- readRDS(data3)$timeline
user3 <- readRDS(data3)$user

links <- readRDS(data)$links
links2 <- readRDS(data2)$links
links3 <- readRDS(data3)$links
...
h3_years <- tibble(order = 3, handle = handle3, years =  unique(lubridate::year(tl3$tsday))[unique(lubridate::year(tl3$tsday)) > 2018])
handle_years <- bind_rows(h1_years, h2_years, h3_years) %>%
...
### `r handle` Daily Rhythym - Day/Hour
...
### `r handle2` Daily Rhythym - Day/Hour
...
### `r handle3` Daily Rhythym - Day/Hour
...
day_count3 <- tl3 %>% 
  group_by(tsday) %>% 
  dplyr::summarise(n3 = dplyr::n())

day_count <- day_count3 %>% 
  left_join(day_count2) %>% 
...
highchart() %>%
  ...
  hc_add_series(day_count, type = "column", hcaes(x = id, y = n3), name = paste0(handle3, " Count")) %>% 
  hc_add_series(fit3, type = "spline", hcaes(x = id, y = .fitted), name = paste0(handle3, " Average")) %>% 
  hc_colors(pal) %>%
...
tweet_type3 <- mutate(tl3, tweet_type = case_when(
  is_retweet ~ "Retweet",
  is_quote ~ "Quote",
  reply_to_user_id == user_id ~ "Self-Reply",
  !is.na(reply_to_user_id) ~ "Reply",
...
tweet_type <- bind_rows(tweet_type1, tweet_type2, tweet_type3)
...
ui3 <- count(tl3, source, sort=TRUE) %>% 
  mutate(source = fct_inorder(source) %>% fct_rev(),
         frac = scales::percent(n/sum(n), accuracy = 1),
         h = handle3)

ui <- bind_rows(ui1, ui2, ui3)

highchart() %>%
  hc_add_series(ui1, "bar", hcaes(x = h, y = n, group = source),
                showInLegend = TRUE) %>% 
  hc_add_series(ui2, "bar", hcaes(x = h, y = n, group = source),
                showInLegend = TRUE) %>%
  hc_add_series(ui3, "bar", hcaes(x = h, y = n, group = source),
                showInLegend = TRUE) %>%
  # hc_xAxis(categories = unique(ui$h)) %>%
  hc_colors(pal) %>%
  hc_tooltip(crosshairs = TRUE,
             headerFormat = "",
             pointFormat = "<b>{point.h} - {point.source}</b><br>
                            <span style=\"color:{point.color}\">\u25CF</span> Tweets: {point.y}<br>
                          <span style=\"color:{point.color}\">\u25CF</span> % of Tweets {point.frac}<br>")
...
### Hashtag Usage - `r handle3`

```{r}
hash <- unlist(tl3$hashtags) %>% 
  discard(is.na) %>% 
  fct_lump(n = 15, ties = "first") %>% 
  table(dnn="hashtag") %>%
  as_tibble() %>%
...
highchart() %>%
  hc_add_series(hash, "bar", hcaes(x = hashtag, y = n), color = pal[3],
                showInLegend = FALSE) %>% 
  hc_xAxis(categories = hash$hashtag) %>% 
  hc_tooltip(crosshairs = TRUE,
             headerFormat = "",
...
highchart() %>%
  hc_add_series(top_url, "bar", hcaes(x = apex, y = n), color = pal[3],
                showInLegend = FALSE) %>% 
  hc_xAxis(categories = top_url$apex) %>% 
  hc_tooltip(crosshairs = TRUE,
             headerFormat = "",
...
if (purrr::is_empty(retweet) == "TRUE") {
  return()
} else {
  retweet <- retweet %>% 
    tolower() %>% 
...
### Replied Users - `r handle3` 

```{r}
reply <- unlist(tl3$reply_to_screen_name) %>% 
  discard(is.na) %>% 
  tolower() %>% 
  fct_lump(n = 10, ties = "first") %>% 
  table(dnn="replied_to_users") %>% 
...
highchart() %>%
  hc_add_series(reply, "bar", hcaes(x = replied_to_users, y = n), color = pal[3],
                showInLegend = FALSE) %>% 
  hc_xAxis(categories = reply$replied_to_users) %>% 
  hc_tooltip(crosshairs = TRUE,
             headerFormat = "",
...
### Retweeted Users - `r handle3`  

```{r}
retweet <- unlist(tl3$retweet_screen_name) %>% 
  discard(is.na) %>% 
  tolower() %>% 
  fct_lump(n = 10, ties = "first") %>% 
  table(dnn="retweeted_users") %>% 
...
highchart() %>%
  hc_add_series(retweet, "bar", hcaes(x = retweeted_users, y = n), color = pal[3],
                showInLegend = FALSE) %>% 
  hc_xAxis(categories = retweet$retweeted_users,
           allowDecimals = FALSE) %>% 
  hc_yAxis(allowDecimals = FALSE) %>%
...
### Quoted Users - `r handle3`  

```{r}
quote <- unlist(tl3$quoted_screen_name) %>% 
  discard(is.na) %>% 
  tolower() %>% 
  fct_lump(n = 10, ties = "first") %>% 
  table(dnn="quoted_users") %>% 
...
highchart() %>%
  hc_add_series(quote, "bar", hcaes(x = quoted_users, y = n), color = pal[3],
                showInLegend = FALSE) %>% 
  hc_xAxis(categories = quote$quoted_users) %>%
  hc_yAxis(allowDecimals = FALSE) %>%
  hc_tooltip(crosshairs = TRUE,
...
```{r, error = TRUE, fig.retina = 1}
knitr::include_graphics(collage2[length(collage2)])
```

:::: 

### Imagery Used - `r handle3`  `r years[3]`  

:::: {style="display: flex;"}

```{r, error = TRUE, fig.retina = 1}
knitr::include_graphics(collage3[length(collage3)])
```
...
```{r, error = TRUE, fig.retina = 1}
knitr::include_graphics(collage2[length(collage2)-1])
```

:::: 

### Imagery Used - `r handle3`  `r years[2]`  

:::: {style="display: flex;"}

```{r, error = TRUE, fig.retina = 1}
knitr::include_graphics(collage3[length(collage3)-1])
```
...
```{r, error = TRUE, fig.retina = 1}
knitr::include_graphics(collage2[length(collage2)-2])
```

:::: 

### Imagery Used - `r handle3`  `r years[1]`  

:::: {style="display: flex;"}

```{r, error = TRUE, fig.retina = 1}
knitr::include_graphics(collage3[length(collage3)-2])
```
...
mutate(tl3, tweet_type = case_when(
  is_retweet ~ "Retweet",
  is_quote ~ "Quote",
  reply_to_user_id == user_id ~ "Self-Reply",
  !is.na(reply_to_user_id) ~ "Reply",
